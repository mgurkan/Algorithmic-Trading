...
GetData<-function(timespan, startdate,issixmonths){

  #1 The average block size in MB
  ...
  AllInfo<-cbind(avgblocksize)
  
  while(ncol(AllInfo)!=1 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
      }
    }
    avgblocksize <- api.blockchain("avg-block-size",list(timespan=timespan,start=startdate,format="csv"))  #2016-03-19
  ...
  #2 The total size of all block headers and transactions. Not including database indexes.
  ...
  AllInfo<-cbind(AllInfo,blocksize)
  while(ncol(AllInfo)!=2 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #3 Cost Per Transaction: Miners revenue divided by the number of transactions
  ...
  AllInfo<-cbind(AllInfo,costpertrans)
  while(ncol(AllInfo)!=3 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #4 Cost % of Transaction Volume: Miners revenue as percentage of the transaction volume
  ...
  AllInfo<-cbind(AllInfo,costpertransper)
  while(ncol(AllInfo)!=4 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #5 Difficulty: A relative measure of how difficult it is to find a new block. The difficulty is adjusted periodically as a function of how much hashing power has been deployed by the network of miners.
  ...
  AllInfo<-cbind(AllInfo,difficulty)
  while(ncol(AllInfo)!=5 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #6 Estimated Transaction Value: The total estimated value of transactions on the Bitcoin blockchain (does not include coins returned to sender as change).
  ...
  AllInfo<-cbind(AllInfo,esttransvol)
  while(ncol(AllInfo)!=6 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #7 Estimated USD Transaction Value: The Estimated Transaction Value in USD value
  ...
  AllInfo<-cbind(AllInfo,esttransvolusd)
  while(ncol(AllInfo)!=7 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #8 Hash Rate: The estimated number of tera hashes per second (trillions of hashes per second) the Bitcoin network is performing.
  ...
  AllInfo<-cbind(AllInfo,hashrate)
  while(ncol(AllInfo)!=8 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #9 Market Capitalization: The total USD value of bitcoin supply in circulation, as calculated by the daily average market price across major exchanges.
  ...
  AllInfo<-cbind(AllInfo,marketcap)
  while(ncol(AllInfo)!=9 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #10 Median Confirmation Time: The median time for a transaction to be accepted into a mined block and added to the public ledger (note: only includes transactions with miner fees).
  ...
  AllInfo<-cbind(AllInfo,medconfirtime)
  while(ncol(AllInfo)!=10 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #11 Confirmed Transactions Per Day: The number of daily confirmed Bitcoin transactions.
  ...
  AllInfo<-cbind(AllInfo,transactions)
  while(ncol(AllInfo)!=11 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #12 Number Of Transactions Excluding Chains Longer Than 100
  ...
  AllInfo<-cbind(AllInfo,transexcludinglonger100)
  while(ncol(AllInfo)!=12 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #13 Number of Transactions Excluding Popular Addresses
  ...
  AllInfo<-cbind(AllInfo,transexcludingpopular)
  while(ncol(AllInfo)!=13 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #14 Average Number Of Transactions Per Block
  ...
  AllInfo<-cbind(AllInfo,transperblock)
  while(ncol(AllInfo)!=14 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  AllInfo<-cbind(AllInfo,transtotal)
  while(ncol(AllInfo)!=15 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #16 Number Of Unique Addresses Used
  ...
  AllInfo<-cbind(AllInfo,uniqueaddresses)
  while(ncol(AllInfo)!=16 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #17 Output Value: The total value of all transaction outputs per day (includes coins returned to the sender as change).
  ...
  AllInfo<-cbind(AllInfo,outputvolume)
  while(ncol(AllInfo)!=17 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #18 Bitcoins in circulation: The total number of bitcoins that have already been mined; in other words, the current supply of bitcoins on the network.
  ...
  AllInfo<-cbind(AllInfo,totalbitcoins)
  while(ncol(AllInfo)!=18 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #19 USD Exchange Trade Volume: The total USD value of trading volume on major bitcoin exchanges
  ...
  AllInfo<-cbind(AllInfo,tradevolume)
  while(ncol(AllInfo)!=19 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #20 Total Transaction Fees: The total value of all transaction fees paid to miners (not including the coinbase value of block rewards).
  ...
  AllInfo<-cbind(AllInfo,transfees)
  while(ncol(AllInfo)!=20 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #21 Total Transaction Fees in USD: The total value of all transaction fees paid to miners (not including the coinbase value of block rewards).
  ...
  AllInfo<-cbind(AllInfo,transfeesusd)
  while(ncol(AllInfo)!=21 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #22 Miners Revenue: Total value of coinbase block rewards and transaction fees paid to miners.
  ...
  AllInfo<-cbind(AllInfo,minersrevenue)
  while(ncol(AllInfo)!=22 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
  ...
  #23
  #Using Quandl
  DollarToChineseYuan<-Quandl("CUR/CNY", api_key="bkosXkpWMbzo6XkWzFKm", start_date=startdate, end_date= Sys.Date()-1-issixmonths)
  ...
  AllInfo<-cbind(AllInfo,DollarToChineseYuanchange)
  while(ncol(AllInfo)!=23 || any(is.na(AllInfo))){
    delay<-DateUnixTimestampConverter(Sys.time())
    while(1==1){
      if(DateUnixTimestampConverter(Sys.time())>delay+60){
        break
...
RealTimeLags<-function(intervalwanted){
  now<-DateUnixTimestampConverter(Sys.time())
  min30<-DateUnixTimestampConverter(Sys.time())-1800
  min60<-DateUnixTimestampConverter(Sys.time())-3600
  day1pre<-DateUnixTimestampConverter(Sys.time())-88200
  day1<-DateUnixTimestampConverter(Sys.time())-86400
  week1pre<-DateUnixTimestampConverter(Sys.time())-606600
  week1<-DateUnixTimestampConverter(Sys.time())-604800
...
GetDailyInput<-function(startdate){
  inputrow<-GetData("2days",startdate,0)
  return(inputrow)
}
...
ReturnofYesterdayTrade<-function(PredProb,RealExRatesChange,intervalwanted){
  ...
  howmanypreds = length(PredProb)
  howmanyrealdata = (24*(60/intervalwanted)-4)
  k=1
  
  for(j in 1: 60){
    for(i in 1:howmanypreds){
      if(PredProb[i] >= threshold){
        capital[k] = capital[k] + capital[k]*RealExRatesChange[howmanyrealdata-howmanypreds+i]
      }
    }
...
WhichModel <- function(PredMatRF, PredMatLR, intervalwanted){
  ...
  if(sum(PredMatLR)==0 && sum(PredMatRF)==0){
    ...
    startdate=DateUnixTimestampConverter(paste0(Sys.Date()-1, sep= " ", "00:45:00"))
    enddate=DateUnixTimestampConverter(paste0(Sys.Date()-1, sep= " ", "23:45:00"))
    
    RealExRates = matrix(0, nrow = (24*(60/intervalwanted)-3), ncol = 2)
    
    RealExRates <- PoloniexBTC_ETH_RATE(intervalwanted,startdate,enddate)
    
    RealExRatesChange = 1
    for(i in 1:(24*(60/intervalwanted)-4)){
      RealExRatesChange[i]=(RealExRates[i+1,2]-RealExRates[i,2])/RealExRates[i,2]
    }
    
    ReturnsFromRF = array(0,60)
    ReturnsFromLR = array(0,60)
...
buyBTC<-function(){
  ...
  if("error" %in% names(resultofbuyingBTC)){
    ...
    amount=BTC_ETH_balance$ETH
    if(amount==0){
      print("BuyBTC cancelation, there is no ETH")
      errorstatus=3
      return(errorstatus)
    }
    else{
...
while(1==1){
  options(scipen = 999)
  now <- DateUnixTimestampConverter(Sys.time()) %% oneday
  
  # to end the day
  if((now >= 85500) && (now < 85600) && isFirst2 == TRUE){
    write.table(paste0("The day has ended. The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
  ...
  # to start the day
  if((now >= 26100) && (now < 26220) && isFirst == TRUE){
    write.table(paste0("The day has begun.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    ...
    write.table(paste0("predcount is: ",predcount),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print(predcount)
  ...
  if((now > 27000) && (now < 27100) && isDataCollectedTrained==FALSE){
    write.table(paste0("Data collection for training is started.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print("6 month data will be fetched")
    #poloniex
    startdate=DateUnixTimestampConverter(paste0(Sys.Date() - trainingperiod - 2, sep= " ", "23:45:00"))
    enddate=DateUnixTimestampConverter(paste0(Sys.Date()-1, sep= " ", "23:45:00")) 
    
    #blockchain.info and ether
    timespan = paste0(trainingperiod+2, "days")
    startdate2 = Sys.Date() - trainingperiod - 3
    AllInfo<-GetData(timespan, startdate2,1)
    ...
    print("6 month data has been fetched, training will begin")
    write.table(paste0("Training is started.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    RFobject=TradewithRandomForest(AllDataFinal)
    LRobject=TradewithLogisticRegression(AllDataFinal)
    write.table(paste0("Training is finished.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
  ...
  # to collect the daily input
  if((now >= 28801) && (now < 28900) && isInitialized==FALSE && isDataCollectedTrained == TRUE){
    write.table(paste0("Data collection for daily input is started.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print("Daily input will be fetched")
    inputrow=GetDailyInput(Sys.Date()-2) #00.00yi gectik
    write.table(paste0("Data collection for daily input is finished.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print("Daily input has been fetched")
    # Dunun prediction matrix ve exchange rate datasina bakarak hangi model ve threshold'a karar ver.
    ret <- WhichModel(PredMatRF, PredMatLR, intervalwanted)
    alpha <- ret[1]
    threshold <- ret[2]
    print("Alpha and threshold are ready")
  ...
  if((now %% min15) < 60 && isInitialized==TRUE){
    isDecided = FALSE
  }
  
  
  
  if((now %% min15) >= 60 && isDecided == FALSE && isInitialized==TRUE && isTrade==FALSE && now>28801){
    write.table(paste0("Each 15 min get the lagged input.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print("15 min loop is okay")
    afterinputrow=cbind(inputrow,RealTimeLags(intervalwanted))
    afterinputrow=as.data.frame(afterinputrow)
    print("Real time lags has been fetched")
    write.table(paste0("Prediction has started.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    RFprediction=PredictwithRF(RFobject,afterinputrow)
    LRprediction=PredictwithLR(LRobject,afterinputrow)
    print("Predictions has been done")
    predcount <- predcount + 1
    write.table(paste0("Prediction has finished.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    
    PredMatRF [predcount] <- RFprediction[1,2]
    PredMatLR [predcount] <- LRprediction
    ...
    if((RFprediction[1,2]*alpha + LRprediction*(1-alpha)) >= threshold){
      ...
      write.table(paste0("The decision has been made, ",isTrade,". Time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
      key="" #fill these
      secret="" #fill these
      balance=api.poloniex(key,secret,"returnBalances")
      titles=names(balance)
      ...
      if(amount>=0.0001){
        ...
        write.table(paste0("Trade of BTC to ETH has finished.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
      }
      else{
        tradestatus=buyBTC()
        if(tradestatus==FALSE){
  ...
  if((now %% min15) > 890 && isTrade == TRUE){
    isTrade = FALSE
    write.table(paste0("We are selling ETH for sure.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
    print("Buying BTC back")
    tradestatus=buyBTC()
    if(tradestatus==FALSE){
      isTrade=TRUE
    }
    ...
    write.table(paste0("Trade of ETH to BTC has finished.The time is: ",Sys.time()),"Simulation.csv",append=TRUE, sep=",",col.names = F, row.names = F)
  }
